...
{
	entityx::Entity tank = entities.get(playerId);
	entityx::Entity aiTank = entities.get(aiId);
	Position::Handle aiPos = aiTank.component<Position>();
	Position::Handle playerPos = tank.component<Position>();
	sf::Vector2f v = (playerPos->m_position) - (aiPos->m_position);
...
{		
	...
	sf::CircleShape obs;
	obs.setRadius(0.0f);
	float theClosestDistance = Math::distance(entities.get(aiId).component<Position>()->m_position, m_obstacles[0].getPosition());
	...
	for (auto &obstacle : m_obstacles)
	{
		...
		if (Math::lineIntersectsCircle(m_ahead, m_halfAhead, obstacle))
		{
			...
			float dist = Math::distance(entities.get(aiId).component<Position>()->m_position, obstacle.getPosition());

			if (dist < theClosestDistance)
			{
				theClosestDistance = dist;
				obs = obstacle;
			}
